...
parser.add_argument('--color_1', default='b', help='영역 1 색을 정합니다.')
parser.add_argument('--color_2', default='g', help='영역 2 색을 정합니다.')
...
for n, st in enumerate(draw_range) :
	...
	# c1
	ax_main.fill_between(xticks, df2_c1.max(axis=1), df2_c1.min(axis=1), facecolor=color_1, alpha=alpha)
	ax_main.plot(df2_c1.max(axis=1), color=color_1, linewidth=3.0)
	ax_main.plot(df2_c1.min(axis=1), color=color_1, linewidth=3.0)
	ax_main.plot(df2_c1.mean(axis=1), color='red', linewidth=3.0)	

	# c2
	ax_main.fill_between(xticks, df2_c2.max(axis=1), df2_c2.min(axis=1), facecolor=color_2, alpha=alpha)
	ax_main.plot(df2_c2.max(axis=1), color=color_2, linewidth=3.0)
	ax_main.plot(df2_c2.min(axis=1), color=color_2, linewidth=3.0)
